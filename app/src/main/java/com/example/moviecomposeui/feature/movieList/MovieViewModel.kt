...
@Inject constructor(private val movieRepository: MovieRepository): ViewModel(){
    val movies: Flow<PagingData<MovieResult>> = Pager(PagingConfig(pageSize = PAGE_SIZE)) {
        DiscoverPagingDataSource(movieRepository)
    }.flow

    val topRatedMovies: Flow<PagingData<MovieResult>> = Pager(PagingConfig(pageSize = PAGE_SIZE)) {
        TopRatedPagingDataSource(movieRepository)
    }.flow
}